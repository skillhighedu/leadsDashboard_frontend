...
import { fetchSelfAnalytics,  } from "@/services/analytics.services";
import type { SelfLeadAnalyticsSummaryResponse } from "@/types/leads";
...
export default function SelfAnalyticsPage() {
  ...
  const [data, setData] = useState<SelfLeadAnalyticsSummaryResponse | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const res = await fetchSelfAnalytics(filters);
      setData(res);
    } catch (err) {
      handleApiError(err);
      setError("Failed to load self analytics.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4 sticky top-0 bg-muted/40 py-4 z-10 backdrop-blur-sm">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Your self performance summary</p>
        </div>
          ...
          <Card className="bg-muted/50 border border-muted-foreground/10 shadow-sm">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2">
                Summary <Badge variant="outline">{data.employee.name}</Badge>
              </CardTitle>
              {renderDateInfo()}
            </CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-sm">
                <div className="flex justify-between bg-white rounded-md p-3 shadow-sm">
                  ...
                  <span className="font-semibold">₹ {data.totals.totalGeneratedAmount}</span>
                </div>
                <div className="flex justify-between bg-white rounded-md p-3 shadow-sm">
                  <span className="text-muted-foreground">Projected</span>
                  <span className="font-semibold">₹ {data.totals.totalProjectedAmount}</span>
                </div>
                <div className="flex justify-between bg-white rounded-md p-3 shadow-sm">
                  <span className="text-muted-foreground">SelfGen Leads</span>
                  <span className="font-semibold">{data.totals.selfGenTrueCount}</span>
                </div>
                <div className="flex justify-between bg-white rounded-md p-3 shadow-sm">
                  <span className="text-muted-foreground">Total Leads</span>
                  <span className="font-semibold">{data.totals.totalLeads}</span>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                {data.statuses.map((s) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium">{String(s.status).replace(/_/g, " ")}</span>
